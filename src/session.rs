...
impl MtrSession {
    ...
    async fn run_ipv4_trace(&mut self, target: Ipv4Addr) -> Result<()> {
        ...
        match self.create_raw_socket() {
            Ok((send_socket, recv_socket)) => {
                ...
                self.run_mtr_algorithm(target, send_socket, recv_socket)
                    .await
            }
            Err(e) => {
    ...
    async fn net_send_batch(&mut self, target: Ipv4Addr, send_socket: &Socket) -> Result<bool> {
        ...
        self.net_send_query(target, send_socket, self.batch_at)
            .await?;
        ...
        if self.batch_at >= (self.args.max_hops as usize) - 1
            ...
            || (self.hops.get(self.batch_at).and_then(|h| h.addr).is_some_and(|addr| {
    ...
    fn save_sequence_with_send_time(&mut self, index: usize, seq: u16, send_time: Instant) {
        ...
        debug!(
            "Saved sequence: seq={}, hop={}, sent_count={}",
            seq,
            index + 1,
            self.hops[index].sent
        );
    }
    ...
    fn mark_sequence_complete(&mut self, seq: u16) -> Option<(usize, Instant)> {
        if let Some(entry) = self.sequence_table.remove(&seq) {
            if entry.transit {
                ...
                if age.as_secs() > 5 {
                    debug!(
                        "Discarding very old response for seq {} (age: {:.1}s)",
                        seq,
                        age.as_secs_f64()
                    );
                    return None;
        ...
        for _ in 0..10 {
            match Self::receive_icmp_response(recv_socket) {
                Ok((source_ip, icmp_type, seq, receive_time)) => {
                    self.net_process_ping(seq, source_ip, icmp_type, receive_time, target)
                        .await;
                }
                ...
                if let Some(ip_packet) = Ipv4Packet::new(&initialized_buffer) {
                    ...
                    if ip_packet.get_next_level_protocol() == IpNextHeaderProtocols::Icmp {
                        ...
                        if let Some(icmp_packet) =
                            IcmpPacket::new(&initialized_buffer[icmp_start..])
                        {
                            ...
                            match icmp_type {
                                IcmpTypes::TimeExceeded => {
                                    ...
                                    if let Some((_orig_id, orig_seq)) =
                                        Self::extract_original_packet_info(icmp_packet.payload())
                                    {
                                ...
                                _ => {
                                    debug!(
                                        "Received unhandled ICMP type: {:?} from {}",
                                        icmp_type, source_ip
                                    );
                                }
                            }
    ...
    fn send_icmp_packet_static(
        ...
        let mut icmp_packet =
            pnet::packet::icmp::echo_request::MutableEchoRequestPacket::new(&mut icmp_buffer)
            ...
            if let Some(orig_ip_packet) = Ipv4Packet::new(&payload[4..]) {
                if orig_ip_packet.get_next_level_protocol() == IpNextHeaderProtocols::Icmp {
                    ...
                    if orig_icmp_start < payload.len() && orig_icmp_start + 8 <= payload.len() {
                        if let Some(orig_icmp) =
                            pnet::packet::icmp::echo_request::EchoRequestPacket::new(
                                &payload[orig_icmp_start..],
                            )
                        {
                            return Some((
                                orig_icmp.get_identifier(),
                                orig_icmp.get_sequence_number(),
                            ));
                        }
                    }
                }
    ...
    async fn run_ipv4_trace_realtime(
        session_arc: std::sync::Arc<std::sync::Mutex<Self>>,
        target: Ipv4Addr,
        args: Args,
    ) -> Result<()> {
        ...
        match socket_result {
            Ok((send_socket, recv_socket)) => {
                info!("Using raw ICMP sockets for real traceroute");
                Self::run_mtr_algorithm_realtime(
                    session_arc,
                    target,
                    send_socket,
                    recv_socket,
    ...
    async fn net_send_batch_realtime(
        ...
        for hop_index in 0..max_hop_to_send {
            ...
            debug!(
                "Sending batch probe: hop={}, TTL={}, seq={}",
                hop_index + 1,
                time_to_live,
                seq
            );
    ...
    async fn net_process_return_realtime(
        ...
        for _ in 0..10 {
            match Self::receive_icmp_response(recv_socket) {
                Ok((source_ip, icmp_type, seq, receive_time)) => {
                    Self::net_process_ping_realtime(
                        session_arc,
                        seq,
                        source_ip,
                        icmp_type,
                        ...
                        if rand::random::<f64>() > packet_loss_chance {
                            ...
                            if hop.addr.is_none() {
                                ...
                                match hop_number {
                                    ...
                                    2..=3 => {
                                        hop.addr =
                                            Some(IpAddr::V4(Ipv4Addr::new(10, 0, hop_number, 1)));
                                        hop.hostname = if !numeric {
                                            Some(format!("core-{}.isp.net", hop_number))
                                        } else {
                                            None
                                        };
                                    }
                                    _ => {
                                        let final_octet =
                                            if hop_number >= 8 { 8 } else { hop_number };
                                        hop.addr =
                                            Some(IpAddr::V4(Ipv4Addr::new(8, 8, 8, final_octet)));
                                        hop.hostname = if !numeric {
                                            Some("dns.google".to_string())
                                        } else {
                                            None
